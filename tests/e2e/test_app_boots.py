"""End-to-end smoke tests for the image-to-ascii-art Streamlit app.

1. ``test_app_imports_and_reaches_dry_run`` runs the script with ``DRY_RUN=true``
   so an import error or a broken dependency upgrade fails fast.
2. ``test_streamlit_server_serves_health`` boots a real Streamlit server and waits
   for the ``/_stcore/health`` endpoint, proving the service comes up end-to-end.
"""
import os
import signal
import socket
...
APP = "image-to-ascii-art-converter-web-ui.py"
...
def _app_env(**extra):
    """Environment matching the Docker deployment contract.

    The app reads ``UPLOAD_DIRECTORY`` with no default (set via ``ENV`` in the
    Dockerfile), so the e2e tests must provide it too.
    """
    env = {**os.environ, "UPLOAD_DIRECTORY": tempfile.mkdtemp(prefix="img2ascii-")}
    env.update(extra)
    return env


def test_app_imports_and_reaches_dry_run():
    env = _app_env(DRY_RUN="true")
    proc = subprocess.run(
        [sys.executable, APP],
        cwd=REPO_ROOT,
        env=env,
